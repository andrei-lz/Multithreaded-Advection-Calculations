...
int main(){
  ...
  /* LOOP 4 */
  /* Cannot be parallelised because the array values have to be written
  in the order that they are stored in memory (next to each other). This creates an
  output dependency (write after write) (a type of loop carried dependency). */
  for (int i=0; i<NX+2; i++){
    for (int j=0; j<NY+2; j++){
  ...
  /* LOOP 5 */
  /* Not parallelisable because you can't skip timesteps as there is a flow dependency
  from one timestep to the other. */
  for (int m=0; m<nsteps; m++){
    ...
    /* LOOP 8 */
    #pragma omp parallel for collapse(2) lastprivate(velocityX)
    for (int i=1; i<NX+1; i++){
      for (int j=1; j<NY+1; j++){
        
        velocityX = 0.0;
        if (y[j] > z0) {
          velocityX = (ustar/k) * log(y[j]/z0);
        } else {
          velocityX = 0;
        }

        dudt[i][j] = -velocityX * (u[i][j] - u[i-1][j]) / dx
    ...
    dt = CFL / ( (fabs(velocityX) / dx) + (fabs(vely) / dy) );
  ...
  should not include the boundary values. */
  float average[NX+2];
